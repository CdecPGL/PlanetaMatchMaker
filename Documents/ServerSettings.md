...
`PMMS_AUTHENTICATION_METHOD` is an optional override. When it is not set, the value loaded from `authentication.method` in `setting.json` is preserved. The production Docker image does not include a default `setting.json`; mount a complete configuration at `/etc/pmms/setting.json` so authentication cannot silently default to `none`.
