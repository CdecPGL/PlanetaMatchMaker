...
# 本番TLS。Docker/server/pmms/pmms.env.exampleからpmms.envを作成し、
# すべての値を置き換えてから実行します。
...
イメージにはSteam認証を選択した `/etc/pmms/setting.json` が同梱されていますが、game ID、Steam AppID、
Publisher Keyは意図的に省略されています。
`PMMS_AUTHENTICATION_GAME_ID`、`PMMS_AUTHENTICATION_STEAM_APP_ID`、
`PMMS_AUTHENTICATION_STEAM_PUBLISHER_KEY` のすべてを環境変数で指定するか、完全な設定ファイルに
差し替えるまでサーバーは起動しません。`--env-file` を使う場合は
[`Docker/server/pmms/pmms.env.example`](Docker/server/pmms/pmms.env.example) を雛形にしてください。設定された
