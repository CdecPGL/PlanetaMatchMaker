...
# 本番TLS。既存の証明書チェーンと秘密鍵をデフォルトパスへマウントします。
...
デフォルトのサーバー設定はTLSを使用します。設定されたパスに証明書と秘密鍵をマウントするか、ローカルで平文TCPを使う場合は `tls.mode` を `"plain"` に明示設定してください。本番用・開発用の証明書設定例は [TLS証明書設定（英語）](Documents/TLSCertificate.md) を参照してください。
